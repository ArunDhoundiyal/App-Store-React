...
class AppStore extends Component {
  state = {categoryTab: tabsList[0].tabId, onClickInput: ''}
  ...
  onClickTabItem = tabId => {
    this.setState({categoryTab: tabId})
  }

  appTabListCategory = categoryTab => {
    const filterAppList = appsList.filter(
      eachList => eachList.category === categoryTab,
    )
    return filterAppList
  }

  render() {
    const {categoryTab, onClickInput} = this.state
    const appCategoryList = this.appTabListCategory(categoryTab)
    const appCategoryInputList = appCategoryList.filter(eachArray =>
      eachArray.appName.toLowerCase().includes(onClickInput.toLowerCase()),
    )
    console.log(appCategoryInputList)

    return (
      <div className="bg-container">
        <h1 className="top-heading">App Store</h1>
        ...
        <ul className="tab-list-container">
          {tabsList.map(tabList => (
            <TabItem
              key={tabList.tabId}
              isActive={tabList.tabId === categoryTab}
              tabList={tabList}
        ...
        <ul className="app-list-container">
          {appCategoryInputList.map(eachAppList => (
            <AppItem key={eachAppList.appId} eachAppList={eachAppList} />
          ))}
        </ul>
      </div>
    )
